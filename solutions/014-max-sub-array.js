...
// I'm still storing values to simplify testing.
export function FindMaxSubArray(arr, k) {
  const res = [];

  while (arr.length > 0 && arr.length >= k) {
    // Get max on array window and shift original.
    res.push(Math.max(...arr.slice(0, k)));
